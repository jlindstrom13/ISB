...
pdf_path = "no_transform_histograms.pdf"

with PdfPages(pdf_path) as pdf:
    for col in stability_df_no_nct.columns:
       
        plt.figure(figsize=(6,4))
        sns.histplot(stability_df_no_nct[col], bins=100, kde=False, color='green')
        plt.yscale("log")
        plt.title(f'Pre-Transformation Histograms: {col}')
        plt.xlim(0,1)
        plt.xlabel('Value')
        plt.ylabel('Count (log scale)')
        pdf.savefig()
